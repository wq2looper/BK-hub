...
local function addFavorite(animalName)
    ...
    if showNotification then
        showNotification("â­ Added to favorites: " .. animalName, COLORS.Cyan)
    end
...
local function removeFavorite(animalName)
    for i, name in ipairs(FAVORITES) do
        if name:lower() == animalName:lower() then
            ...
            saveFavorites()
            if showNotification then
                showNotification("âŒ Removed from favorites: " .. animalName, COLORS.Red)
            end
            return true
        end
    end
...
function ANTI_RAGDOLL.Disable()
    ...
    showNotification("âœ— Anti-Ragdoll Disabled", COLORS.Red)
end
...
local function tpNearPlotIfFar(animalData)
    ...
    -- If close enough already â†’ skip
    ...
    -- Vector from plot center to player (playerâ€™s viewing side)
...
local function teleportToAnimal(animalData)
    ...
    if not character or not character:FindFirstChild("HumanoidRootPart") then
        showNotification("âš ï¸ Character not found!", COLORS.Warning)
        return false
    end
    ...
    local plot = workspace.Plots:FindFirstChild(animalData.plot)
    if not plot then
        showNotification("âš ï¸ Plot not found!", COLORS.Warning)
        return false
    end
    ...
    showNotification(string.format("ðŸš€ Teleported to %s", animalData.name), COLORS.Cyan)
    return true
end
...
local function teleportToHighest()
    ...
    if CONFIG.FAVORITES_PRIORITY_ENABLED and #FAVORITES > 0 then
        ...
        if favAnimal then
            showNotification("ðŸ”– Teleporting to priority: " .. favAnimal.name, COLORS.Yellow)
            teleportToAnimal(favAnimal)
            return
        else
            showNotification("âš ï¸ No favorites found, using highest gen", COLORS.TextDim)
        end
    end
...
local function initializePlotScanner(statLabels)
    local plots = workspace:WaitForChild("Plots", 8)
    if not plots then
        warn("âš ï¸ Plots folder not found!")
        return
    end
...
local function createAnimalEntry(animalData, parent)
    ...
    local name = createElement("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = (isFav and "â­ " or "") .. animalData.name,
...
local function createFavoritesTab(parent)
    ...
    local apply = createElement("TextLabel", {
        ...
        Position = UDim2.new(0, 15, 0, 10),
        BackgroundTransparency = 1,
        Text = "â­ Apply Favorites Priority",
        TextColor3 = COLORS.Yellow,
        TextSize = 15,
        Font = Enum.Font.GothamBold,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    toggleButton.MouseButton1Click:Connect(function()
        ...
        if showNotification then
            showNotification(
                CONFIG.FAVORITES_PRIORITY_ENABLED and "âœ… Favorites Priority Enabled" or "âŒ Favorites Priority Disabled",
